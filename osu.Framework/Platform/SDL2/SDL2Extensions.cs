...
namespace osu.Framework.Platform.SDL2
{
    public static class SDL2Extensions
    {
        public static Key ToKey(this SDL.SDL_Keysym sdlKeysym)
        {
            ...
            bool numLockOn = sdlKeysym.mod.HasFlagN(SDL.SDL_Keymod.KMOD_NUM) || RuntimeInfo.IsApple;
        ...
        public static WindowState ToWindowState(this SDL.SDL_WindowFlags windowFlags)
        {
            ...
            if (windowFlags.HasFlagN(SDL.SDL_WindowFlags.SDL_WINDOW_FULLSCREEN_DESKTOP) ||
                windowFlags.HasFlagN(SDL.SDL_WindowFlags.SDL_WINDOW_BORDERLESS) ||
                windowFlags.HasFlagN(SDL.SDL_WindowFlags.SDL_WINDOW_MAXIMIZED) && RuntimeInfo.OS == RuntimeInfo.Platform.MacOsx)
                return WindowState.FullscreenBorderless;

            if (windowFlags.HasFlagN(SDL.SDL_WindowFlags.SDL_WINDOW_MINIMIZED))
                return WindowState.Minimised;

            if (windowFlags.HasFlagN(SDL.SDL_WindowFlags.SDL_WINDOW_FULLSCREEN))
                return WindowState.Fullscreen;

            if (windowFlags.HasFlagN(SDL.SDL_WindowFlags.SDL_WINDOW_MAXIMIZED))
