...
namespace osu.Framework.Input.Handlers.Mouse
{
    internal class OsuTKRawMouseHandler : OsuTKMouseHandlerBase, IHasCursorSensitivity, INeedsMousePositionFeedback
    {
                    ...
                    host.InputThread.Scheduler.Add(scheduled = new ScheduledDelegate(delegate
                    {
                        ...
                        if ((MouseInWindow.Value || lastEachDeviceStates.Any(s => s != null && s.Buttons.HasAnyButtonPressed)) && host.Window.Focused)
                        {
                            ...
                            for (int i = 0; i < newRawStates.Count; i++)
                            {
                                ...
                                HandleState(newState, lastState, rawState.Flags.HasFlagN(MouseStateFlags.MoveAbsolute));
        ...
        private Vector2 getUpdatedPosition(MouseState state, OsuTKMouseState lastState)
        {
            ...
            if (state.Flags.HasFlagN(MouseStateFlags.MoveAbsolute))
            {
                ...
                {
                    Rectangle screenRect = state.Flags.HasFlagN(MouseStateFlags.VirtualDesktop)
