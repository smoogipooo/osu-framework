    ...
    public abstract class TabControl<T> : CompositeDrawable, IHasCurrentValue<T>, IKeyBindingHandler<PlatformAction>
    {
        ...
        protected TabControl()
        {
            ...
            if (Dropdown != null)
            {
                ...
                Trace.Assert(Dropdown.Header.Anchor.HasFlagN(Anchor.x2), $@"The {nameof(Dropdown)} implementation should use a right-based anchor inside a TabControl.");
                Trace.Assert(!Dropdown.Header.RelativeSizeAxes.HasFlagN(Axes.X), $@"The {nameof(Dropdown)} implementation's header should have a specific size.");
            }
