...
namespace osu.Framework.Graphics.Containers
{
    ...
    public class FillFlowContainer<T> : FlowContainer<T>, IFillFlowContainer where T : Drawable
    {
        ...
        private Vector2 spacingFactor(Drawable c)
        {
            ...
            if (c.Anchor.HasFlagN(Anchor.x2))
                result.X = 1 - result.X;
            if (c.Anchor.HasFlagN(Anchor.y2))
                result.Y = 1 - result.Y;
            return result;
        }
        ...
        protected override IEnumerable<Vector2> ComputeLayoutPositions()
        {
            ...
            if (max == Vector2.Zero)
            {
                ...
                max.X = AutoSizeAxes.HasFlagN(Axes.X) ? float.MaxValue : s.X;
                max.Y = AutoSizeAxes.HasFlagN(Axes.Y) ? float.MaxValue : s.Y;
            }
            ...
            for (int i = 0; i < children.Length; ++i)
            {
                ...
                if (c.Anchor.HasFlagN(Anchor.x1))
                    // Begin flow at centre of row
                    result[i].X += rowOffsetsToMiddle[rowIndices[i]];
                else if (c.Anchor.HasFlagN(Anchor.x2))
                    // Flow right-to-left
                    result[i].X = -result[i].X;

                if (c.Anchor.HasFlagN(Anchor.y1))
                    // Begin flow at centre of total height
                    result[i].Y -= height / 2;
                else if (c.Anchor.HasFlagN(Anchor.y2))
                    // Flow bottom-to-top
