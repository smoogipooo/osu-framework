    ...
    public abstract partial class CompositeDrawable : Drawable
    {
        ...
        protected virtual bool CheckChildrenLife()
        {
            ...
            for (int i = 0; i < internalChildren.Count; i++)
            {
                ...
                anyAliveChanged |= state.HasFlagN(ChildLifeStateChange.MadeAlive) || state.HasFlagN(ChildLifeStateChange.MadeDead);

                if (state.HasFlagN(ChildLifeStateChange.Removed))
                    i--;
            }
        ...
        public override float Width
        {
            get
            {
                if (!isComputingChildrenSizeDependencies && AutoSizeAxes.HasFlagN(Axes.X))
                    updateChildrenSizeDependencies();
                return base.Width;
            }
        ...
        public override float Height
        {
            get
            {
                if (!isComputingChildrenSizeDependencies && AutoSizeAxes.HasFlagN(Axes.Y))
                    updateChildrenSizeDependencies();
                return base.Height;
            }
        ...
        private Vector2 computeAutoSize()
        {
            ...
            try
            {
                ...
                foreach (Drawable c in aliveInternalChildren)
                {
                    ...
                    if (!c.BypassAutoSizeAxes.HasFlagN(Axes.X))
                        maxBoundSize.X = Math.Max(maxBoundSize.X, cBound.X);

                    if (!c.BypassAutoSizeAxes.HasFlagN(Axes.Y))
                        maxBoundSize.Y = Math.Max(maxBoundSize.Y, cBound.Y);
                }

                if (!AutoSizeAxes.HasFlagN(Axes.X))
                    maxBoundSize.X = DrawSize.X;
                if (!AutoSizeAxes.HasFlagN(Axes.Y))
        ...
        private void updateAutoSize()
        {
            ...
            autoSizeResizeTo(new Vector2(
                AutoSizeAxes.HasFlagN(Axes.X) ? b.X : base.Width,
                AutoSizeAxes.HasFlagN(Axes.Y) ? b.Y : base.Height
