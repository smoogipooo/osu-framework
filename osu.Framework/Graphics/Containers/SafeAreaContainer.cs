...
namespace osu.Framework.Graphics.Containers
{
    ...
    public class SafeAreaContainer : Container
    {
        ...
        private MarginPadding getPadding()
        {
            ...
            return new MarginPadding
            {
                Left = SafeAreaOverrideEdges.HasFlagN(Edges.Left) ? nonSafeLocalSpace.TopLeft.X : Math.Clamp(localTopLeft.X, 0, DrawSize.X),
                Right = SafeAreaOverrideEdges.HasFlagN(Edges.Right) ? DrawRectangle.BottomRight.X - nonSafeLocalSpace.BottomRight.X : Math.Clamp(DrawSize.X - localBottomRight.X, 0, DrawSize.X),
                Top = SafeAreaOverrideEdges.HasFlagN(Edges.Top) ? nonSafeLocalSpace.TopLeft.Y : Math.Clamp(localTopLeft.Y, 0, DrawSize.Y),
                Bottom = SafeAreaOverrideEdges.HasFlagN(Edges.Bottom) ? DrawRectangle.BottomRight.Y - nonSafeLocalSpace.BottomRight.Y : Math.Clamp(DrawSize.Y - localBottomRight.Y, 0, DrawSize.Y)
            };
        }
    }
}
