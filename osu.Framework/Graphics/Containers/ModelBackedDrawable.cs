...
namespace osu.Framework.Graphics.Containers
{
    ...
    public abstract class ModelBackedDrawable<T> : CompositeDrawable where T : class
    {
        ...
        private void updateDrawable()
        {
            if (model == null)
            {
                // For a null model, we want to display nothing
                loadDrawable(null);
                return;
            }
            ...
            if (TransformImmediately)
                loadDrawable(() => CreateDrawable(null));

            loadDrawable(() => CreateDrawable(model));
        }

        private void loadDrawable([NotNull] Func<Drawable> createDrawableFunc)
        {
        ...
        /// </summary>
        /// <param name="model">The model that the <see cref="Drawable"/> should represent. May be null during a load if <see cref="TransformImmediately"/> is <code>true</code>.</param>
        [NotNull]
