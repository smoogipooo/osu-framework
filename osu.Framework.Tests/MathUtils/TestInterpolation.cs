...
namespace osu.Framework.Tests.MathUtils
{
    ...
    public class TestInterpolation
    {
        ...
        public void TestGenericInterpolation()
        {
            ...
            Assert.AreEqual(10, InterpolationCache<int>.ValueAt(0.1, 0, 100, 0, 1));
            Assert.IsTrue(Precision.AlmostEquals(0.01, InterpolationCache<double>.ValueAt(0.1, 0, 0.1, 0, 1)));

            // Implementations inside struct
            Assert.AreEqual(new MarginPadding(10), InterpolationCache<MarginPadding>.ValueAt(0.1, new MarginPadding(0), new MarginPadding(100), 0, 1));
            Assert.AreEqual(new TestClassWithValueAt(50), InterpolationCache<TestClassWithValueAt>.ValueAt(10, new TestClassWithValueAt(0), new TestClassWithValueAt(100), 0, 20));

            // Without implementations
            Assert.Throws<TypeInitializationException>(() => InterpolationCache<TestClassWithoutValueAt>.ValueAt(0, new TestClassWithoutValueAt(), new TestClassWithoutValueAt(), 0, 0));
        }
